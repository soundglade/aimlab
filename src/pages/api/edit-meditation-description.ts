...
export default async function handler(
  ...
  try {
    const { meditationId, ownerKey, description } = req.body;
    ...
    // Update description (or remove it if description is null)
    if (description === null) {
    ...
    return res
      .status(500)
      .json({ error: "Failed to update meditation description" });
  }
}
